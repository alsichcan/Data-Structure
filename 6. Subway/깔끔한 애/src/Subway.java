...
public class Subway {
    ...
    private static HashMap<String, Long> distance;
    private static PriorityQueue<adjacentST> adList;
    ...
    private static void register(String data) throws Exception {
        ...
        while ((line = br.readLine()) != null) {
            ...
            NumMap.get(lineData[0]).getLine(lineData[0]).add_adjacent(new adjacentST(lineData[0], lineData[1], Long.parseLong((lineData[2]))));
        }
    ...
    private static void search(String start, String dest, Boolean minTrans) {
        ...
        adList = new PriorityQueue<>(); // 접근할 수 있는 역들을 거리에 따라 배열하는 Heap구조
        ...
        // 거리 update 및 방문 표시
        distance.put(stl.getNum(), new Long(0));
        ...
        while(!NumMap.get(adList.peek().getDest()).getName().equals(dest)){

            // 현재 상태에서 최단 시간에 도달할 수 있는 역이 목적지와 같을 때까지

            // 현재 접근할 수 있는 역 중 가장 가까운 역까지의 edge를 선택한다.
            adjacentST temp = adList.poll();
        ...
        // 목적지까지의 최소 경로 시간
        long time = distance.get(last.getDest());
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////
    private static void update(stLine stn, adjacentST ad, long time){
        long t = 0;
        if(time == -1){
            t = ad.getTime();
        } else{

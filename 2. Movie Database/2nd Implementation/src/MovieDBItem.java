...
public class MovieDBItem implements Comparable<MovieDBItem> {
    ...
    // 리턴값이 양수이면 this가 other보다 순서가 뒤라는 뜻이고
    // 리턴값이 0이면 this가 other과 같다는 의미이며
    // 리턴값이 음수이면 this가 other보다 순서가 앞이라는 뜻이다.
    ...
    public int compareTo(MovieDBItem other) {
        return this.getGenre().compareTo(other.getGenre()) * 10000 + this.getTitle().compareTo(other.getTitle());
    }
